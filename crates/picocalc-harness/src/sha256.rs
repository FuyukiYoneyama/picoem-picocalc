...
//! `generic-array` stack) into a crate that otherwise depends only on
...
mod tests {
    ...
    fn known_vectors() {
        ...
        // 56 bytes — exercises the extra-padding-block branch.
        assert_eq!(
    ...
    fn multi_block() {
        ...
        // Cross-checked against `sha256sum` on the same byte sequence.
        assert_eq!(
            sha256_hex(&data),
