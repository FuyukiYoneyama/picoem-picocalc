...
impl Emulator {
    ...
    /// Like [`Self::step_until`], but repeatedly executes serial quanta until
    /// a caller-specified cap, while preserving the boundary stop conditions.
    ///
    /// The batch advances in whole configured quanta by repeatedly invoking
    /// [`Self::step_serial_with_external`] with `exact_idle_fast_forward =
    /// true`. `max_batch_cycles` limits only repeated running-core dispatches.
    /// An exact idle fast-forward made by an individual quantum may cross that
    /// internal limit, but never `external_event_cycle`; this preserves the
    /// exact semantics of [`Self::step_until`].
    ///
    /// Stops when any of the following occurs:
    ///
    /// * `max_batch_cycles` cumulative running-core progress is reached;
    /// * `external_event_cycle` is reached;
    /// * no progress is made (`consumed == 0`);
    /// * core 0 enters a fatal exception (`IPSR == 2 || IPSR == 3`);
    /// * `sys_clk_hz` changes (`ClockTree::sys_clk_hz` rebase).
    ///
    /// If the emulator is currently both-blocked on entry, this is identical to
    /// [`Self::step_until`] so the long exact idle fast-forward remains
    /// available without batching.
    pub fn step_until_batched(
        &mut self,
        external_event_cycle: u64,
        max_batch_cycles: u64,
        ...
        let max_batch_cycles = max_batch_cycles.max(1);
        let start_cycle = self.clock.cycles;

        let both_cores_blocked = (self.cores[0].is_halted() || self.bus.wfe_waiting[0])
            && (self.cores[1].is_halted() || self.bus.wfe_waiting[1]);
        if both_cores_blocked {
            return self.step_until(external_event_cycle);
        }

        let batch_end = start_cycle.saturating_add(max_batch_cycles);
        let batch_cap = external_event_cycle.min(batch_end);

        let mut total_consumed = 0u64;
        let entry_sys_clk_hz = self.bus.clock_tree.sys_clk_hz;

        while self.clock.cycles < batch_cap {
            let consumed = self.step_serial_with_external(Some(external_event_cycle), true);
            #[cfg(feature = "behavior-trace")]
            self.observe_behavior_trace();
            ...
            let c0_ipsr = self.cores[0].regs.xpsr & 0x1ff;
            if c0_ipsr == 2 || c0_ipsr == 3 {
                break;
            }

            if self.bus.clock_tree.sys_clk_hz != entry_sys_clk_hz {
                break;
            }
        }
...
mod stage5_lib_residue {
    ...
    #[test]
    fn step_until_batched_matches_repeated_step_until_and_advances_multiple() {
        let mut batched = EmulatorBuilder::new(Config::default())
            .step_quantum(1)
            .build()
            .unwrap();

        let mut reference = EmulatorBuilder::new(Config::default())
            .step_quantum(1)
            .build()
            .unwrap();
        ...
        let target = 48;
        let batched_consumed = batched
            .step_until_batched(target, 32)
            .expect("Serial batched stepping is infallible");

        let mut reference_consumed = 0;
        while reference_consumed < batched_consumed {
            let consumed = reference
                .step_until(target)
                .expect("Serial step_until is infallible");
            assert!(consumed > 0);
            reference_consumed = reference_consumed.saturating_add(consumed);
        }

        assert_eq!(batched_consumed, reference_consumed);
        assert_eq!(batched_consumed, 32);
        assert_eq!(batched.clock.cycles, reference.clock.cycles);
        assert_eq!(batched.cores[0].regs.pc(), reference.cores[0].regs.pc());
        assert!(batched_consumed > 1);
    }

    #[test]
    fn step_until_batched_stops_at_external_boundary_or_batch_cap() {
        let mut emu = EmulatorBuilder::new(Config::default())
            .step_quantum(1)
            .build()
            .unwrap();
        ...
        let consumed = emu
            .step_until_batched(5, 3)
            .expect("Serial batched stepping is infallible");

        assert_eq!(consumed, 3);
        assert_eq!(emu.clock.cycles, 3);
        let consumed = emu
            .step_until_batched(5, 10)
            .expect("Serial batched stepping is infallible");
        assert_eq!(consumed, 2);
        assert_eq!(emu.clock.cycles, 5);
    }

    #[test]
    fn step_until_batched_stops_on_fatal_exception() {
        let mut emu = EmulatorBuilder::new(Config::default())
            .step_quantum(8)
            .build()
            .unwrap();
        ...
        let stack_top = 0x2002_0000u32;
        for i in 0..16 {
            emu.bus.write32(vtor + (i as u32) * 4, 0);
        }
        emu.bus.write32(vtor + 3 * 4, handler | 1);
        emu.bus.ppb[0].vtor = vtor;
        emu.bus.write16(0x2000_4000, 0xBE00); // BKPT #0 -> HardFault
        emu.bus.write16(handler, 0x4770); // BX LR
        emu.cores[0].regs.msp = stack_top;
        emu.cores[0].regs.set_sp(stack_top);
        emu.cores[1].regs.msp = 0x2002_2000;
        emu.cores[1].regs.set_sp(emu.cores[1].regs.msp);
        emu.cores[0].regs.set_pc(0x2000_4000);
        emu.cores[0].regs.r[13] = emu.cores[0].regs.msp;
        emu.cores[1].regs.r[13] = emu.cores[1].regs.msp;

        emu.cores[1].regs.set_pc(0x2000_2000);
        emu.cores[1].regs.r[13] = emu.cores[1].regs.msp;
        emu.bus.write16(0x2000_2000, 0xBF00);

        let consumed = emu
            .step_until_batched(1000, 64)
            .expect("Serial batched stepping is infallible");
    ...
    #[test]
    fn step_until_batched_preserves_unbatched_exact_idle_when_both_cores_blocked() {
        let mut emu = EmulatorBuilder::new(Config::default())
            .step_quantum(1)
            .build()
            .unwrap();

        // Both cores blocked on entry, so batched stepping must delegate to
        // `step_until` and retain exact idle fast-forward behavior.
        emu.cores[0].halt();
        emu.halt_core1();
        let target = 200u64;
        let consumed = emu
            .step_until_batched(target, 64)
            .expect("Serial batched stepping is infallible");

        assert!(consumed >= 65, "expected exact-idle-like jump > 64, got {consumed}");
        assert_eq!(consumed, target);
        assert_eq!(emu.clock.cycles, target);
    }

    #[test]
    fn step_until_batched_stops_on_sys_clk_change() {
        use crate::bus::clocks::CLK_SYS_DIV;
        use crate::bus::CLOCKS_BASE;
        ...
        const SYS_DIV_ADDR: u32 = CLOCKS_BASE + CLK_SYS_DIV;

        emu.cores[0].regs.set_pc(0x2000_1000);
        emu.cores[0].regs.msp = 0x2002_0000;
        emu.cores[0].regs.r[13] = emu.cores[0].regs.msp;
        emu.cores[0].regs.r[0] = 0x0002_0000;
        emu.cores[0].regs.r[1] = SYS_DIV_ADDR;
        emu.bus.write16(0x2000_1000, 0x6008);
        emu.bus.write16(0x2000_1002, 0xBF00);

        let before_sys_clk = emu.bus.clock_tree.sys_clk_hz;
        let consumed = emu
            .step_until_batched(4_000_000, 64)
            .expect("Serial batched stepping is infallible");

        assert!(consumed > 0);
        assert_ne!(emu.bus.clock_tree.sys_clk_hz, before_sys_clk);
        assert!(
            consumed < 64,
            "expected sub-batch stop on clock-tree rebase, got full batch {consumed}"
        );
    }

    #[test]
    fn step_until_batched_running_to_blocked_transition_allows_exact_single_call_jump() {
        use crate::peripherals::timer::{ALARM0_OFFSET, INTE_OFFSET};
        ...
        emu.cores[0].regs.msp = 0x2002_0000;
        emu.cores[0].regs.r[13] = emu.cores[0].regs.msp;
        emu.bus.write16(0x2000_1000, WFI);
        emu.halt_core1();
        ...
        emu.bus
            .timer
            .write32(ALARM0_OFFSET, 1024, 0, emu.bus.master_cycle, sys_hz);

        let timer_deadline = emu
            .bus
            .next_scheduled_lazy_deadline()
            .expect("timer alarm should be scheduled");
        let external = timer_deadline + 10_000;

        let batched_consumed = emu
            .step_until_batched(external, 64)
            .expect("Serial batched stepping is infallible");
        ...
        let mut reference = EmulatorBuilder::new(Config::default())
            ...
            .build()
            .unwrap();
        reference.cores[0].regs.set_pc(0x2000_1000);
        reference.cores[0].regs.msp = 0x2002_0000;
        reference.cores[0].regs.r[13] = reference.cores[0].regs.msp;
        reference.bus.write16(0x2000_1000, WFI);
        reference.halt_core1();
        let sys_hz = reference.bus.clock_tree.sys_clk_hz;
        reference.bus.timer.write32(
            INTE_OFFSET,
            1,
            0,
            reference.bus.master_cycle,
            sys_hz,
        );
        reference.bus.timer.write32(
            ALARM0_OFFSET,
        ...
        let mut ref_consumed = 0u64;
        while ref_consumed < batched_consumed {
            let c = reference
                .step_until(external)
                .expect("Serial step_until is infallible");
            if c == 0 {
            ...
            ref_consumed = ref_consumed.saturating_add(c);
            if ref_consumed >= external {
                break;
            }
        }

        assert!(batched_consumed > 64);
        assert_eq!(batched_consumed, ref_consumed);
        assert_eq!(emu.clock.cycles, reference.clock.cycles);
        assert_eq!(emu.bus.master_cycle, timer_deadline);
    }

    #[cfg(feature = "behavior-trace")]
    #[test]
    fn step_until_batched_behavior_trace_snapshot_matches_repeated_reference() {
        let mut batched = EmulatorBuilder::new(Config::default())
            .step_quantum(1)
            .build()
            .unwrap();
        ...
        let setup = |emu: &mut Emulator| {
            ...
            emu.cores[0].regs.msp = 0x2002_0000;
            emu.cores[0].regs.r[13] = emu.cores[0].regs.msp;
            emu.enable_behavior_trace().unwrap();
        };
        ...
        let batched_trace = batched
            ...
            .expect("behavior tracer enabled")
            .sha256;
