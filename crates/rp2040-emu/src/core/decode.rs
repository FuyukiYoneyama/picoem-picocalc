...
/// Conservative production oracle for one-cycle batch execution.
///
/// `true` means this decode entry has no known bus side-effects, does not
/// branch, advances PC by exactly two bytes, and consumes one architectural
/// cycle in the current decoder model.
#[cfg(feature = "exact-event-batching")]
pub(crate) fn classify_batch_safe_one_cycle(hw0: u16, _hw1: u16, wide: bool) -> bool {
    if wide {
        return false;
    }
    if is_wide(hw0) {
    ...
    match hw0 >> 11 {
        // shifts, add/sub, mov / cmp / add / sub immediate
        0b00000..=0b00111 => true,
        // data-processing register where bit10 == 0
        0b01000 => hw0 & (1 << 10) == 0,
        // ADR / ADD SP immediate
...
impl CortexM0Plus {
    ...
    fn populate_decode_cache<B: CoreBus>(&mut self, bus: &mut B, pc: u32) -> DecodedOp {
        ...
        #[cfg(feature = "exact-event-batching")]
        if classify_batch_safe_one_cycle(hw0, hw1, wide) {
            flags |= DecodedOp::FLAG_BATCH_SAFE_ONE_CYCLE;
        }
    ...
    /// Count consecutive cached one-cycle batch-safe instructions starting
    /// from the current PC.
    ///
    /// The scan is cache-only (no bus/memory reads), and stops when:
    /// * `pc == stop_pc`
    /// * `max_cycles` would be exceeded
    /// * decode cache miss
    /// * entry is not marked batch-safe one-cycle
    #[cfg(feature = "exact-event-batching")]
    pub(crate) fn count_cached_batch_safe_one_cycle_instructions(
        &self,
        stop_pc: u32,
        max_cycles: u64,
    ) -> u64 {
        ...
        while cycles < max_cycles && pc != stop_pc {
            if !is_cacheable_pc(pc) {
                break;
            }
            let slot = ((pc >> 1) & CACHE_INDEX_MASK) as usize;
            let entry = self.decode_cache[slot];
            if entry.tag != pc || !entry.is_batch_safe_one_cycle() {
                break;
            }
    ...
    /// Execute one cached batch-safe instruction if present.
    ///
    /// Read-only from bus perspective: no `CoreBus` calls, no bus-based
    /// fault delivery, and no active-PC publication. Returns:
    /// * `None` on decode-cache miss
    /// * `Some(false)` when cached entry exists but is not batch-safe one-cycle
    /// * `Some(true)` and `self.cycles += 1` after successful execution
    #[cfg(feature = "exact-event-batching")]
    pub(crate) fn step_cached_batch_safe_one_cycle(&mut self) -> Option<bool> {
        let pc = self.regs.pc();
        if !is_cacheable_pc(pc) {
            return None;
        }
        ...
        if !entry.is_batch_safe_one_cycle() {
            return Some(false);
        }

        // Match `decode_execute`: PC is architecturally advanced before the
        // handler runs, while `current_instr_addr` retains the address used by
        // PC-relative operations such as ADR.
        self.current_instr_addr = pc;
        self.regs.set_pc(pc.wrapping_add(2));
        let consumed = self.execute_batch_safe_one_cycle(entry.hw0);
        debug_assert!(consumed == 1);
        self.cycles = self.cycles.wrapping_add(consumed as u64);
        Some(consumed > 0)
    }

    /// Execute the small safe subset without bus interaction.
    #[cfg(feature = "exact-event-batching")]
    fn execute_batch_safe_one_cycle(&mut self, opcode: u16) -> u32 {
        match opcode >> 11 {
            0b00000 => self.thumb16_lsl_imm(opcode),
            0b00001 => self.thumb16_lsr_imm(opcode),
            0b00010 => self.thumb16_asr_imm(opcode),
            ...
            0b00110 => self.thumb16_add_imm8(opcode),
            0b00111 => self.thumb16_sub_imm8(opcode),
            0b01000 => self.thumb16_data_processing(opcode),
            0b10100 => self.thumb16_adr(opcode),
            0b10101 => self.thumb16_add_sp_imm(opcode),
            _ => 0,
        }
...
mod batch_classifier_tests {
    use super::CortexM0Plus;
    use super::classify_batch_safe_one_cycle;
    use crate::bus::DecodedOp;

    fn op(prefix: u16, imm: u16) -> u16 {
        (prefix << 11) | imm
    }

    fn set_cached_entry(core: &mut CortexM0Plus, pc: u32, opcode: u16, batch_safe: bool) {
        let slot = ((pc >> 1) & super::CACHE_INDEX_MASK) as usize;
        let mut flags = 0u8;
        if batch_safe {
            flags |= DecodedOp::FLAG_BATCH_SAFE_ONE_CYCLE;
        }
        core.decode_cache[slot] = DecodedOp {
            tag: pc,
            hw0: opcode,
            hw1: 0,
            flags,
    ...
    #[test]
    fn classify_batch_safe_one_cycle_positive_examples() {
        assert!(classify_batch_safe_one_cycle(op(0b00000, 0), 0, false));
        assert!(classify_batch_safe_one_cycle(op(0b00111, 0), 0, false));
        assert!(classify_batch_safe_one_cycle(
            0b01000_0_000_000000,
            0,
            false
        ));
        assert!(classify_batch_safe_one_cycle(op(0b10100, 0x1), 0, false));
        assert!(classify_batch_safe_one_cycle(op(0b10101, 0x2), 0, false));
    }

    #[test]
    fn classify_batch_safe_one_cycle_negative_examples() {
        assert!(!classify_batch_safe_one_cycle(op(0b11110, 0), 0, false));
        let special_dp = (0b01000u16 << 11) | (1u16 << 10);
        assert!(!classify_batch_safe_one_cycle(special_dp, 0, false));
        assert!(!classify_batch_safe_one_cycle(op(0b11010, 0), 0, false));
        assert!(!classify_batch_safe_one_cycle(op(0b00110, 0), 0xF000, true));
    }

    #[test]
    fn count_cached_batch_safe_one_cycle_instructions_scans_contiguous_window() {
        let mut core = CortexM0Plus::new();
        let base = 0x2000u32;
        core.regs.set_pc(base);

        set_cached_entry(&mut core, base, op(0b00000, 0), true);
        set_cached_entry(&mut core, base + 2, op(0b00100, 0), true);
        set_cached_entry(&mut core, base + 4, op(0b01010, 0), false);
        set_cached_entry(&mut core, base + 6, op(0b00000, 0), true);

        assert_eq!(
            core.count_cached_batch_safe_one_cycle_instructions(base + 8, 10),
            2
        );
        assert_eq!(
            core.count_cached_batch_safe_one_cycle_instructions(base + 4, 10),
            2
        );
        assert_eq!(
            core.count_cached_batch_safe_one_cycle_instructions(base + 8, 1),
            1
        );
        assert_eq!(
            core.count_cached_batch_safe_one_cycle_instructions(base + 8, 0),
            0
        );
        assert_eq!(
            core.count_cached_batch_safe_one_cycle_instructions(base + 2, 10),
            1
        );
    }

    #[test]
    fn step_cached_batch_safe_one_cycle_executes_state_and_cycles() {
        let mut core = CortexM0Plus::new();
        let pc = 0x2000u32;
        core.regs.set_pc(pc);
        core.cycles = 7;
        set_cached_entry(&mut core, pc, op(0b00100, 0x12), true);

        assert_eq!(core.step_cached_batch_safe_one_cycle(), Some(true));
        assert_eq!(core.regs.r[0], 0x12);
        assert_eq!(core.regs.pc(), pc + 2);
        assert_eq!(core.cycles, 8);
        assert_eq!(
            core.count_cached_batch_safe_one_cycle_instructions(pc + 2, 10),
            0
        );
    }

    #[test]
    fn step_cached_batch_safe_one_cycle_handles_miss_and_nonsafe_entry() {
        let mut core = CortexM0Plus::new();
        let pc = 0x2000u32;
        core.regs.set_pc(pc);

        assert_eq!(core.step_cached_batch_safe_one_cycle(), None);

        set_cached_entry(&mut core, pc, op(0b01010, 0), false);
        let old_pc = core.regs.pc();
        let old_cycles = core.cycles;
        assert_eq!(core.step_cached_batch_safe_one_cycle(), Some(false));
        assert_eq!(core.regs.pc(), old_pc);
        assert_eq!(core.cycles, old_cycles);
    }
}
