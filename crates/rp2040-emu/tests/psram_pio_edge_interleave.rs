...
fn bulk_quantum_write_frame_lands_correct_byte_with_psram_attached() {
    ...
    {
        // The drained program is now blocked on its next empty-TX PULL.
        // A further quantum must exercise OPT2-F rather than merely
        // passing through its device-level helpers in isolation.
        run_steps(&mut emu, 1);
        let snapshot = emu.exact_bulk_prototype_snapshot();
        assert!(snapshot.outer_calls > 0);
        assert!(snapshot.update_gpio_calls_elided > 0);
    }
}
